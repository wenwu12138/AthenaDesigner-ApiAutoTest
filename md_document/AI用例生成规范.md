...
- `detail` 只写场景，不写变量
- `detail`、`name`、`description`、`title`、`zh_CN`、`zh_TW`、`en_US` 等文案字段必须使用标准 UTF-8 文本，禁止把 `????`、乱码占位串或不可读字符写入 YAML 文件
- 每次生成或批量改写后，必须至少扫描 3 类问题：问号残缺词、不可打印控制字符、异常非中文高位字符；发现后必须先修复再提交 YAML
- 禁止把 `自动补齐缺失说明，保留当前执行与断言逻辑` 作为长期保留文案留在稳定集；它只能作为临时占位，必须在当轮补成真实场景说明
- 对已存在 YAML 做批量替换或编码修复时，必须先限定作用范围，只允许修改 `detail`、`name`、`description` 等文案字段，禁止无范围替换整份 YAML
- `assert` 第一阶段统一只保留 `$.code == 0`

### 3.1 detail 文案质量规则

`detail` 是稳定集可读性的基础，也是 Allure 标题和问题排查入口。后续生成和修复必须满足：

- `detail` 必须能独立说明“这个接口在闭环里做什么”
- 优先使用“动作 + 对象 + 目的/校验点”的句式，例如“查询项目分页结果并缓存首个项目编码用于后续任务闭环校验”
- 如果接口当前被 `is_run: false` 跳过，`detail` 仍然要写真实原因，不能只写“跳过”或空泛描述
- 禁止使用乱码、问号残缺词、拼音缩写和无法落地的占位描述

### 3.1.1 其他文案字段规则

除了 `detail` 外，以下字段同样视为必须自检的文案字段：

- 请求体里的 `name`、`description`、`title`
- 多语言字段 `zh_CN`、`zh_TW`、`en_US`
- 其他直接出现在 YAML 中、会进入后端存储或页面展示的说明性字段

规则：

- 这些字段禁止残留历史乱码，例如 `AI瑕嗙...`、`绱㈠紩椤?`、`Զȱʧ˵ǰִ߼`
- 如果字段是创建闭环的一部分，文案必须贴合闭环语义，不能沿用临时占位文本
- 多语言字段至少保证 `zh_CN` 可读；若 `zh_TW`、`en_US` 无法保证质量，应显式写清晰文本，不允许留乱码

## 3.2 文案乱码问题成因回灌

本轮排查发现，`data/ai` 中文案字段乱码主要来自 4 类问题：

1. 编码链路不统一  
   某些批量生成或后处理步骤把 UTF-8 文本按 GBK/GB18030 再次转码，造成 `鎸夋...` 这类历史乱码。

2. 批量修复范围过大  
   没有限定只处理 `detail` 字段，导致原本正常的中文描述和后续新增闭环文案被误伤。

3. 临时占位没有回填  
   使用了“自动补齐缺失说明，保留当前执行与断言逻辑”这类过渡文案，但后续没有逐条回填真实语义。

4. 只修 `detail`，没有同步扫描其他文案字段  
   部分 `name`、`description`、`title`、`zh_CN`、`zh_TW` 仍残留历史乱码，说明自检范围不完整。

规则更新：

- 以后所有 AI 生成和批量修复脚本统一按 UTF-8 读写 YAML
- 批量修复前必须先做小样本预览，确认不会把正常中文再次转坏
- 任何“占位说明”必须在当轮收口，不允许跨轮长期滞留
- 如果某段文案已经发生多次错误转码，优先按接口语义直接重写，不再反复猜测编码
- 自检范围必须覆盖整个文案字段集合，而不是只扫 `detail`
- 每轮提交前至少执行一次“全文件文案扫描”，确认 `detail/name/description/title/多语言字段` 均无乱码后再进入自检或回归
